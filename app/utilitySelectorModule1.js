define(["dojo/on", "dojo/_base/declare", "dojo/dom-construct",
    ...
    "esri/graphic", "app/utilitySelectorConfig", "esri/Color", "esri/SpatialReference"],
    function (on, declare, domConstruct,
        QueryTask, Query, Polyline, CartographicLineSymbol,
        Graphic, UtilitySelectorConfig, Color, spatialReference) {
        return declare(null, {
            ...
            go: function () {
                ...
                $.map(resultSet, (line, idx) => {
                    let toMerge = [];
                    $.map(line.geometry.paths, (path, pathIdx) => {
                        $.merge(toMerge, path);
                    });
                ...
                lineSymbol.setWidth(2.25);

                //ctx.utilityGraphicsLayer.add(new Graphic(postMileMarkerPolyline, lineSymbol));

                app.map.graphics.add(new Graphic(postMileMarkerPolyline, lineSymbol));
                app.map.setExtent(postMileMarkerPolyline.getExtent(),false);
            ...
            populatePostmileDdls: function (evt, ctx) {
                ...
                query.orderByFields = [ctx._config.postmileField];
